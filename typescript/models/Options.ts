...
export interface Options {
     ...
     * All values except for _FERRIES_ will also be ignored for non-motorized profiles such as _BICYCLE_ or _PEDESTRIAN_.
    ...
    currency?: string;
    /**
     * Specifies a maximum distance between consecutive events along the route [m].
     * When set, additional events are inserted to ensure that no gap between two events exceeds the given distance.
     * These inserted events do not carry a specific type such as _MANEUVER_EVENTS_ or _BORDER_EVENTS_.
     * They appear in the event list with the common event properties only (latitude, longitude,
     * distanceFromStart, travelTimeFromStart, etc.).
     * 
     * The inserted events are not equidistant and are not placed at the midpoint of a gap.
     * Their positions depend on the set of explicitly requested events. Adding or removing requested
     * event types may change the positions of the inserted events. Start and destination will always be present in the event list.
     * 
     * If no other event types are requested, the response will solely contain these basic events.
     * 
     * For a long route and a low value a significant number of events will be inserted.
     * @type {number}
     * @memberof Options
     */
    maximumDistanceBetweenEvents?: number;
}
...
export function OptionsFromJSONTyped(json: any, ignoreDiscriminator: boolean): Options {
    ...
    return {
        ...
        'maximumDistanceBetweenEvents': json['maximumDistanceBetweenEvents'] == null ? undefined : json['maximumDistanceBetweenEvents'],
    };
}
...
export function OptionsToJSON(value?: Options | null): any {
    ...
    return {
        ...
        'maximumDistanceBetweenEvents': value['maximumDistanceBetweenEvents'],
    };
}
